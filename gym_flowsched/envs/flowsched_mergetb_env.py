# Added by TJ Kim (2.22.21)
# Uses data and is single path with multiple layers
...
class FlowSchedMergeTBEnv(Env):
    ...
    def __init__(self):
        ...
        self.nL = 4 # changed for single path
        self.nF = 10
        self.nS = 20
        self.nA = 3
    ...
    def step(self, a):
        ...
        # TJ Edit - Make all flows unified into a single path 
        if self.num_flows < self.nF:
            
            path = np.array([1, 1, 1, 1]) # first path of the 6-link diamond network
            assert path.size == self.nL # Check that path is configured correctly
        ...
        # print("rate dim", self.rate_link.shape)
        # TJ
        self.wt = wt
        # print("weight dim", wt[0])

        if ( self.rm_size == np.zeros((self.nL, self.nF)) ).all() and self.num_flows >= self.nF:
            for i in range(self.nL):
                print('Flow time on link {} is: {}'.format(i, self.flow_time_link[i]))
